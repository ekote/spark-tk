...
package org.trustedanalytics.sparktk.models.timeseries.ar

import breeze.linalg._
import com.cloudera.sparkts.models.{Autoregression, ARModel => SparkTsArModel}
...
object ArModel {
   ...
   *
   * @param ts A frame to train the model on.
   * @param timeseriesColumn Name of the column that contains the time series values.
   * @param tsMaxLag The maximum lag order for the dependent (time series) variable.
  ...
  def train(ts: Seq[Double],
            ...
            tsMaxLag: Int,
            noIntercept: Boolean = false): ArModel = {
//    require(ts != null, "frame is required")
    require(StringUtils.isNotEmpty(timeseriesColumn), "timeseriesColumn must not be null nor empty")
//
//    val trainFrameRdd = new FrameRdd(ts.schema, ts.rdd)
//    trainFrameRdd.cache()
//
//    val yVector = TimeSeriesFunctions.getVectorFromFrame(trainFrameRdd, timeseriesColumn)
//
//    val ts = new org.apache.spark.mllib.linalg.DenseVector(yVector.toArray)

    val ts_vector = new org.apache.spark.mllib.linalg.DenseVector(ts.toArray)
    val arModel = Autoregression.fitModel(ts_vector, tsMaxLag, noIntercept)

    ArModel(timeseriesColumn, tsMaxLag, noIntercept, arModel)
  }

//  /**
//   * Load a ARModel from the given path
//   * @param tc TkContext
//   * @param path location
//   * @return
//   */
//  def load(tc: TkContext, path: String): ARModel = {
//    tc.load(path).asInstanceOf[ARModel]
//  }
...
//    val m: ARModelTkMetaData = SaveLoad.extractFromJValue[ARModelTkMetaData](tkMetadata)
//
//    // Define the spark-ts ARX model
//    val sparkModel = new SparkTsARModel(m.c, m.coefficients, m.yMaxLag, m.xMaxLag, includesOriginalX)
//
//    // Create the ARModel to return
//    ARModel(m.timeseriesColumn, m.xColumns, m.yMaxLag, m.xMaxLag, m.noIntercept, sparkModel)
//  }
}

case class ArModel private[ar] (timeseriesColumn: String,
                                tsMaxLag: Int,
                                noIntercept: Boolean = false,
                                arModel: SparkTsArModel) extends Serializable {

  /**
   * An intercept term, zero if none desired
   */
  def c: Double = arModel.c
   ...
   */
  def coefficients: Seq[Double] = arModel.coefficients
...
//    val maxLag = max(ARModel.yMaxLag, ARModel.xMaxLag)
//    val predictions = Array.fill(maxLag) { null } ++ ARModel.predict(yVector, xMatrix).toArray
...
//    val tkMetadata = ARModelTkMetaData(timeseriesColumn, xColumns, tsMaxLag, xMaxLag, noIntercept, ARModel.c, ARModel.coefficients)
//    TkSaveLoad.saveTk(sc, path, ARModel.formatId, ARModel.currentFormatVersion, tkMetadata)
//  }
}
 ...
 */
case class ARModelTkMetaData(timeseriesColumn: String,
                              yMaxLag: Int,
                              noIntercept: Boolean,
                              c: Double,
